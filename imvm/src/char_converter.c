...
int main(int argc, char* argv[] ) {
	...
	for ( i = 1; i < argc; i++ ) {
		...
		output = (unsigned char *) calloc( 1, sizeof(unsigned char) * output_sz);
		output_sz = hex2bin(argv[i], input_sz, output, output_sz);
		if (output_sz < 0) {
			printf("Invalid input : hex2bin return code : %d", output_sz);
			free(output);
			return output_sz;
		}
		...
		if (hex_output_sz < 0 ) {
			printf("Invalid input to bin2hex: return code %d ", hex_output_sz);
			free(hex_output);
			return hex_output_sz;
		}
